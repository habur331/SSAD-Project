...
public class ReportByStudentIDVisitor implements Visitor{
    ...
    public void visitAttendanceDB(AttendanceDB db) {
        ...
        if (student.isPresent())
        {
            ...
            attendances.forEach(i -> System.out.println( i.getDate() + " " + i.getSubjectName() + (i.getAttended() ? " were present ":" were absent ")));
        }
    }
    ...
    public void visitPaymentDB(PaymentBookDB db) {
        if (student.isPresent()) {
            ...
            paymentBook.forEach(i -> System.out.println( i.getRepaid() ? (" has dept " + (i.getPrice() - i.getInvestment())) : ("paid the bill in full") ));
        }
    }
